...
void AST::mainHeader(const std::vector<std::string>& profileName,
    ...
    std::vector<std::string>::const_iterator j = fileName.begin();
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    for(std::list<AST*>::iterator obj = child.begin(); obj != child.end(); obj++){
      if((*obj)->tag == "function" && (*obj)->getChild("name")->getName() == "main"){
        AST* block = (*obj)->getChild("block");
        std::list<AST*>::iterator itr = block->child.end();
        itr--;
        while(itr != block->child.begin()){
          if((*itr)->tag == "return"){
            break;
          }
          itr--;
        ...
        for(unsigned long i = 0; i < profileName.size(); i++){
          std::string insertString = "std::cout << " + profileName[i] + " << std::endl;\n    "; 
          block->child.insert(itr, new AST(token, insertString));
        }
      }
    }
}
...
void AST::functionCount(const std::string& profileName) {
    ...
    for(std::list<AST*>::iterator obj = child.begin(); obj != child.end(); obj++){
      if((*obj)->tag == "function"){
        AST* block = (*obj)->getChild("block");
        std::list<AST*>::iterator itr = block->child.begin();
        std::string functionName = (*obj)->getChild("name")->getName();
        std::string insertString = " " + profileName + ".count(__LINE__, \"" + functionName + "\");";
        block->child.insert(++itr, new AST(token, insertString));
      }
    }
}
...
void AST::lineCount(const std::string& profileName) {
    ...
    for(std::list<AST*>::iterator obj = child.begin(); obj != child.end(); obj++){
      if((*obj)->nodeType == category){
        if((*obj)->tag == "expr_stmt"){
          std::string insertString = " " + profileName + ".count(__LINE__);";
          std::list<AST*>::iterator itr2 = obj;
          child.insert(++itr2, new AST(token, insertString));
        }
        else{
          if(isStopTag((*obj)->tag)){

          }
          else{
            (*obj)->lineCount(profileName);
          }
        }
      }
    }
