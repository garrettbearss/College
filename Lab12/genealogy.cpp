...
public:
   ...
   // Allows for the modification of firstName_
   void setFirstName(string input){firstName_=input;}

   virtual void accept(class PersonVisitor *)=0;  
   virtual ~Person(){}
private:
   // Changed from const to regular string in order to modify firstName_
   string firstName_;
   Person *spouse_;
   Person *father_;
   Person *mother_;
...
public:
  void visit(Man *m) override {
    if(m->getFather() != nullptr){
      m->setFirstName(m->getFirstName()+" "+m->getFather()->getFirstName());
    }
  }
  void visit(Woman *w) override {
    if(w->getMother() != nullptr){
      w->setFirstName(w->getFirstName()+" "+w->getMother()->getFirstName());
    }
  }
};
